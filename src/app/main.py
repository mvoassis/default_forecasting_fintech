...
app = FastAPI()
ohe = load_model('models/ohe')
scaler = load_model('models/scaler')
model = load_model('models/XGBoost_Opt')
# model = load_model('')

origins = [
    "http://localhost",
    "https://docs.ufpr.br",  # Substitua pelo seu domínio
]
...
def prediction_model(person_age, person_income, person_home_ownership,person_emp_length,
    ...
    return {'result' : int(pred),
             'proba_0' : float(proba_0),
             'proba_1' : float(proba_1) }